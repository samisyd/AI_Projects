...
A Streamlit application that extracts graph data (entities and relationships) from text input using LangChain and OpenAI's GPT models, and generates interactive knowledge graphs.
...
6. Explore the interactive knowledge graph:
   ...
   - Use the filter menu to focus on specific nodes and edges.
...
- `generate_knowledge_graph.py`: Core logic for extracting and visualizing the knowledge graph.
- `knowledge_graph.ipynb`: Jupyter notebook for development and experimentation.
- `main.py`: Basic entry point (not used in the app).
- `requirements.txt`: Python dependencies.
- `pyproject.toml`: Project configuration.
- `output/`: Directory containing generated HTML graph files.
